...
class LabDataStore:
    ...
    def write_session(
        ...
        *,
        ...
        for name, wf in (waveforms or {}).items():
            fname = f"{name}.npz"
        ...
        for name, series in (scalar_series or {}).items():
            fname = f"{name}.csv"
            self._write_scalar_csv(sdir / fname, series)
            channels.append(
                ChannelMeta(
                    name=name,
                    kind="scalar_series",
    ...
    def load_waveform(
        ...
        *,
        ...
        if t_start is not None or t_end is not None:
            ...
            mask = (t >= lo) & (t <= hi)
            t, v = t[mask], v[mask]
            raw = raw[mask] if raw is not None else None
        return Waveform(
            time_s=t,
            voltage_v=v,
