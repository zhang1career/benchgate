...
documentation (in the benchgate install / source tree):
  docs/CLI_REFERENCE.md     all leaf commands (44)
...
def cmd_lab_thermal_watch(args: argparse.Namespace) -> int:
    params: dict = {"design_dir": args.design}
    if args.instrument:
        params["instrument"] = args.instrument
    if args.interval_s is not None:
        params["interval_s"] = args.interval_s
    ...
    if args.baseline_file:
        params["baseline_file"] = args.baseline_file
    for key in ("delta_warn", "delta_fail", "k_sigma_warn", "k_sigma_fail"):
        val = getattr(args, key, None)
        if val is not None:
            params[key] = val
    params["require_baseline"] = not bool(getattr(args, "no_require_baseline", False))
    ...
    if getattr(args, "apply_calibration", False):
        params["apply_calibration"] = True
    result = dispatch("lab_thermal_watch", params)
    print(json.dumps(result, indent=2, ensure_ascii=False))
    runs = result.get("runs") or []
    # A poll that never captured is a failure, not a clean run.
    if any(not r.get("ok") or r.get("severity") == "fail" for r in runs):
        return 1
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    lthc.add_argument("--frames", type=int, default=None)
    lthc.add_argument("--reduce", default=None, choices=["max", "mean", "last", "median"])
    ...
    ltha.add_argument("--frames", type=int, default=None)
    ltha.add_argument("--reduce", default=None, choices=["max", "mean", "last", "median"])
    ...
    lthw = th_sub.add_parser("watch", help="Poll thermal alert (does not join KiCad watch_loop)")
    _add_design_arg(lthw)
    lthw.add_argument("--instrument", default=None)
    lthw.add_argument("--interval-s", dest="interval_s", type=float, default=None)
    lthw.add_argument("--max-iterations", dest="max_iterations", type=int, default=0)
    lthw.add_argument("--quiet", action="store_true")
    lthw.add_argument("--frames", type=int, default=None)
    lthw.add_argument("--reduce", default=None, choices=["max", "mean", "last", "median"])
    lthw.add_argument("--baseline-file", dest="baseline_file", default=None)
    lthw.add_argument("--delta-warn", dest="delta_warn", type=float, default=None)
    lthw.add_argument("--delta-fail", dest="delta_fail", type=float, default=None)
    lthw.add_argument("--k-sigma-warn", dest="k_sigma_warn", type=float, default=None)
    lthw.add_argument("--k-sigma-fail", dest="k_sigma_fail", type=float, default=None)
    lthw.add_argument("--no-require-baseline", dest="no_require_baseline", action="store_true")
    lthw.add_argument("--homography-file", dest="homography_file", default=None)
    lthw.add_argument("--apply-calibration", dest="apply_calibration", action="store_true")
    lthw.set_defaults(func=cmd_lab_thermal_watch)
